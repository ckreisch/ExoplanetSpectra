...
class LightCurve:
    ...
    def __init__(self, PathToLC, wave_bin_size):
        ...
        for i in range(new_obj_num):
            for j in range(wave_bin_size):
                Path_to_files[j] = '{}/{}'.format(PathToLC,
                                   files_list[i*wave_bin_size + j])
...
class LightCurveData:
    ...
    def __init__(self, Path_to_files):
        ...
        #sample the first file just to have info that are in common for all files
        ...
        for i in range(len_file-1):
            for j in range(param_num):
                for k in range(file_num):
                   av_param_list[j][i] = av_param_list[j][i]
                                         + param_list[k][j][i]/file_num
    ...
    def new_time_bin(self, bin_size):
        ...
        for i in range(new_size):
            for j in range(bin_size):
                new_time[i] = new_time[i] 
                              + time[i*bin_size + j]/bin_size
                new_flux[i] = new_flux[i] 
                              + flux[i*bin_size + j]/bin_size
